...
bool load_data_mc(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        try {
            uint32_t addr = stoul(addr_str, nullptr, 16);
            int32_t value = stol(value_str, nullptr, 16); // Signed for memory
...
void decode() {
    ...
    // S-type
        ...
        else if (func3 == stoul(func3_map["sd"], nullptr, 2)) ctrl.mem_size = "DOUBLE";
        else {
    ...
    if (ctrl.alu_op == "BEQ" || ctrl.alu_op == "BNE" || ctrl.alu_op == "BLT" || ctrl.alu_op == "BGE") {
        reg_a_val = reg_file[rs1];  // First operand for comparison
        reg_b_val = reg_file[rs2];  // Second operand for comparison
    }
    else if (ctrl.alu_op == "STORE") {
        reg_a_val = reg_file[rs1];  // Base address
        // Store value will be obtained from rs2 in memory_access
    } 
    else if (ctrl.alu_op != "LUI" && ctrl.alu_op != "EXIT") {
        reg_a_val = reg_file[rs1];  // First operand
        reg_b_val = ctrl.use_imm ? rm : reg_file[rs2];  // Second operand (immediate or register)
    }
    else {
        reg_a_val = 0;  // Not used for LUI/EXIT
        reg_b_val = rm;  // Immediate value for LUI
    }
...
void execute() {
    ...
    else if (ctrl.alu_op == "DIV") rz = b ? a / b : 0;  // Prevent division by zero
    else if (ctrl.alu_op == "REM") rz = b ? a % b : 0;  // Prevent modulo by zero
    else if (ctrl.alu_op == "AND") rz = a & b;
    else if (ctrl.alu_op == "OR") rz = a | b;
    else if (ctrl.alu_op == "XOR") rz = a ^ b;
    else if (ctrl.alu_op == "SLL" || ctrl.alu_op == "SLLI") rz = a << (b & 0x1F);  // Only use lower 5 bits for shift amount
    else if (ctrl.alu_op == "SRL" || ctrl.alu_op == "SRLI") rz = static_cast<uint32_t>(a) >> (b & 0x1F);  // Logical shift (use unsigned)
    else if (ctrl.alu_op == "SRA" || ctrl.alu_op == "SRAI") rz = a >> (b & 0x1F);  // Arithmetic shift (preserves sign)
    else if (ctrl.alu_op == "SLT") rz = (a < b) ? 1 : 0;
    else if (ctrl.alu_op == "LUI") rz = b;  // Upper immediate loaded directly
    else if (ctrl.alu_op == "AUIPC") rz = pc - 4 + b;  // PC + upper immediate
    ...
    // Memory operations
    else if (ctrl.alu_op == "LOAD" || ctrl.alu_op == "STORE") rz = a + b;  // Address calculation
    ...
    // Prepare memory address and data for load/store
    if (ctrl.alu_op == "LOAD" || ctrl.alu_op == "STORE") {
        mar = rz;  // Memory address
        if (ctrl.alu_op == "STORE") {
            rm = reg_file[rs2];  // Value to store
        }
    }
