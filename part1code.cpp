...
void dataDirectives(vector<string> dataInst) {
    ...
    for (size_t i = 0; i < dataInst.size(); i++) {
        ...
        // Handle labels (e.g., "myLabel: .word 42")
        ...
        if (colonPos != string::npos) {
            ...
            data = afterColon;
            ss.str(data); // Reset stringstream with data after colon
            ss.clear();
        }
        ...
        if (temp_word == ".asciiz" || temp_word == ".string") {
            string remainder;
            getline(ss, remainder); // Get the rest of the line
            ...
            string s = remainder.substr(1, remainder.length() - 2); // Remove quotes
            for (char c : s) {
        ...
        // Handle other data types (.word, .half, .byte)
        else {
            while (ss >> temp_word) {
                try {
                    long long value = stoll(temp_word); // Use stoll for larger range
                    ...
                    output_error.AlterError(INVALID_DATA, "Invalid number: " + temp_word);
                    continue;
                } catch (const out_of_range&) {
                    output_error.AlterError(INVALID_DATA, "Number out of range: " + temp_word);
                    continue;
                }
            }
        }
